...
export function HabitCard({
  ...
  return (
    <div
      className={cn(
        "relative w-full rounded-2xl border border-zinc-800 bg-zinc-900 transition-all",
    ...
    >
      {/* Main tap area — marks habit complete */}
      <button
        onClick={handleTap}
        ...
        className={cn(
          ...
          !habit.completedToday && !isLocked && "active:scale-[0.98] transition-transform",
        )}
      >
      ...
      {/* Edit link — sits outside the button so it's valid HTML */}
      <Link
        href={`/habits/edit?id=${habit.id}`}
        className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 text-zinc-700 hover:text-zinc-400 transition-colors"
